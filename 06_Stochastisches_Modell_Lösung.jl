...
β = let
	wärmekapazität = 6.131644992014602 # in (W a) / (m² K)
	abstrahlung_a = 210.3 # in W / m²
	abstrahlung_b = 2.15 # in W / (m² °C)
	abstrahlung(T) = abstrahlung_a + abstrahlung_b * T
	
	T1 = 5.0 # Temperaturen in °C
	T2 = 10.0
	T3 = 15.0
	
	τ = 6 # in a
	
	β = -(T3 / τ) * wärmekapazität / (abstrahlung(T3) * (1 - T3 / T1) * (1 - T3 / T2))
end
...
let
	...
	δ = @. β * (1 - T / T1) * (1 - T / T2) * (1 - T / T3)
	plot(T, δ, xguide = L"T", yguide=  L"\delta", label = "")
end
...
function f(t, T)
	...
	r = 1 + 0.001 * cos(2 * pi * t/ 1.0e5) # milankovic
	return abstrahlung(T) / wärmekapazität * (r / (1 + β * (1 - T / T1) * (1 - T / T2) * (1 - T / T3)) - 1)
end
...
# ╔═╡ 64498ee1-f930-4b03-bbf6-90a78db1cc72
md"""
Die beiden Temperaturen $T_1 = 5°C$ und $T_3 = 15°C$ sind stationäre Zustände,
da die rechte Seite $f$ dort Null ist. Sie sind stabil, da eine kleine Änderung
der Temperatur dazu führt, dass sie sich wieder dem stationären Zustand annähert.

Betrachten wir bspw. $T_1$. Ist die Temperatur $T$ etwas kleiner als $T_1$, dann ist $f(0, T) > 0$. Die Temperatur wächst also und nähert sich damit $T_1$. Ist die Temperatur leicht größer als $T_1$, dann ist $f(0, T) < 0$ und die Temperatur nähert sich wiederum $T_1$.
"""
...
# ╔═╡ c56bce4b-2cc0-4dd7-a593-780c081a1e2c
md"""
#### Kleinere Amplituden des Mylankovic-Zyklus

Wenn der externe zyklische Einfluss schächer ist, tritt immer noch ein Wechsel zwischen den stationären Zuständen auf. Die Periode ist allerdings nicht mehr klar durch die externe Anregung festgelegt.
"""

# ╔═╡ 8b84d5a3-dbb8-4048-8ef0-32e5e0bbb464
function f_schwach(t, T)
	wärmekapazität = 6.131644992014602 # in (W a) / (m² K)
	abstrahlung_a = 210.3 # in W / m²
	abstrahlung_b = 2.15 # in W / (m² °C)
	abstrahlung(T) = abstrahlung_a + abstrahlung_b * T
	...
	r = 1 + 0.0005 * cos(2 * pi * t/ 1.0e5) # milankovic
	return abstrahlung(T) / wärmekapazität * (r / (1 + β * (1 - T / T1) * (1 - T / T2) * (1 - T / T3)) - 1)
end

# ╔═╡ 2a682fd3-b737-47f6-b488-e9e9975a7800
let
	abbildung = plot(xguide = L"Zeit $t$ in Jahren",
					 yguide = L"Temperatur $T$ in K")
	...
	for T0 in [14.0] # in °C
		zeit, temperatur = euler_maruyama(t0, tf, T0, schrittweite, f_schwach, σ)
		plot!(abbildung, zeit, temperatur, label = "\$T(t_0) = $(T0)\$ K")
	end
...
# ╔═╡ 16d35154-dc2b-46fa-a31c-75dbc8af1dd9
md"""
#### Stärkeres Rauschen

Einen ähnlichen Effekt beobachten wir, wenn die externe Anregung gleich bleibt aber das Rauschen stärker wird.
"""

# ╔═╡ 4621b538-0be4-404a-8511-5873e66a0192
let
	abbildung = plot(xguide = L"Zeit $t$ in Jahren",
					 yguide = L"Temperatur $T$ in K")

	Random.seed!(1)
	σ = sqrt(0.25) # sqrt(K² / year)
	t0 = 0.0
	tf = 3.0e5
	schrittweite = 1.0 # = h, in Jahren
...
# ╔═╡ 9a24b4e2-cc91-454d-a9de-2a9117849367
md"""
#### Schwächeres Rauschen

Schwächeres Rauschen führt dazu, dass kein Wechsel zwischen den stationären Zuständen mehr auftritt.
"""

# ╔═╡ b078b230-99e0-416a-afac-e2ff42461fe6
let
	abbildung = plot(xguide = L"Zeit $t$ in Jahren",
					 yguide = L"Temperatur $T$ in K")

	Random.seed!(1)
	σ = sqrt(0.05) # sqrt(K² / year)
	t0 = 0.0
	tf = 3.0e5
	schrittweite = 1.0 # = h, in Jahren
...
# ╔═╡ c17d650d-577f-487f-baae-23169e9e7fc8
md"""
#### Fazit

Das Größen von stochastischem Rauschen und externer Anregung müssen zueinander passen, damit der Wechsel zwischen den stationären Zuständen mit klarer Frequenz auftreten kann.
"""
