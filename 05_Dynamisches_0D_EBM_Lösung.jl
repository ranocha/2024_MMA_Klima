...
# ╔═╡ e70e440b-9291-4f77-9442-21281e5c5615
...
# ╔═╡ b806923a-e3a7-4e05-9e70-aca6f40af83a
wärmekapazität = 1.935e8 / sekunden_pro_jahr # in (W a) / (m² K)
...
# ╔═╡ 5fc72363-f49a-4587-961f-160bc1fa2866
solarkonstante = 1361 # = S₀, in W / m²

# ╔═╡ da739425-b077-4966-a0ce-8edcfc8b7d78
albedo = 0.3 # = α

# ╔═╡ 80a78853-e2eb-463b-b01c-47b75838a91c
abstrahlung_a = 210.3 # in W / m²

# ╔═╡ 7accb2cd-d751-4f79-b733-ecec2bc5ea12
abstrahlung_b = 2.15 # in W / (m² °C)

# ╔═╡ b0ebf2ae-da81-4f6f-a6bd-c89218b6f684
co2_konzentration = 315.0 # in ppm

# ╔═╡ b4731111-eafb-4c20-b1b3-498633b33e39
abstrahlung_Δa = log(co2_konzentration / 315.0) * 5.35 # in W / m²

# ╔═╡ d41b534e-47ac-4978-9365-b98ef8d04ba3
temperatur_gleichgewicht = (0.25 * solarkonstante * (1 - albedo) - 
								(abstrahlung_a - abstrahlung_Δa)) / abstrahlung_b
...
let
	...
	t = range(t0, tf, length = 200) # in Jahren
	temperatur(t, T0) = temperatur_gleichgewicht + (T0 - temperatur_gleichgewicht) * 
						exp(-t / (wärmekapazität / abstrahlung_b))
	
	T0 = 12.0 # in °C
	plot!(abbildung, t, temperatur.(t, T0), label = "\$T(t_0) = $(T0)\$°C")
...
# ╔═╡ 5558614b-8e70-4460-a528-d608660e61cb
let
	abbildung = plot(xguide = L"Zeit $t$ in Jahren",
					 yguide = L"Temperatur $T$ in °C")
	...
	f(t, T) = (0.25 * solarkonstante * (1 - albedo) - 
				((abstrahlung_a - abstrahlung_Δa) + abstrahlung_b * T)) / wärmekapazität
		
	T0 = 12.0 # in °C
	h = 0.5 # in Jahren
	zeit = [t0]
	temperatur = [T0]
	let t = zeit[end], T = temperatur[end] # Schritt 1
		push!(zeit, t + h)
		push!(temperatur, T + h * f(t, T))
	end
	let t = zeit[end], T = temperatur[end] # Schritt 2
		push!(zeit, t + h)
		push!(temperatur, T + h * f(t, T))
	end
	scatter!(abbildung, zeit, temperatur, label = "\$T(t_0) = $(T0)\$°C")
...
# ╔═╡ e4e2ad00-0133-44db-9999-1564196452b7
md"""
### Stabilität der numerischen Ergebnisse

- Bis zu einer Schrittweite von ungefähr 4 Jahren passen die numerischen Ergebnisse qualitativ gut zu der analytischen Lösung.
- Bei einer Schrittweite von 5 Jahren konvergiert die Temperatur nicht mehr monoton gegen die Gleichgewichtstemperatur (auf einer Zeitspanne von 100 Jahren).
- Bei einer Schrittweite von 6 Jahren divergieren die numerischen Ergebnisse sogar.

Dies ist ein generelles Prinzip in der Numerik:
- Bei einem expliziten Verfahren wie dem Euler-Verfahren kann die Schrittweite $h$ wegen der Stabilität beschränkt sein.
- Es gibt natürlich bessere Verfahren - aber die Wahl eines guten Verfahrens und guter numerischer Parameter erfordert oft trotzdem ein gewisses Fachwissen.
"""
